  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full lg:w-72 pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#D4A017] focus:border-transparent outline-none"
          />
          {busqueda && (
            <button
              onClick={() => setBusqueda('')}
      ...
      {tipoActivo === 'todas' && !busqueda && destacadas.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold text-[#0A1628] mb-6">Destacados</h2>
              ...
              >
                <Card className="overflow-hidden hover:shadow-xl transition-all h-full">
                  <div className={`h-48 relative ${
                    pub.tipo === 'evento' 
                      ? 'bg-gradient-to-br from-[#D4A017] to-[#B8860B]' 
                      : 'bg-gradient-to-br from-[#0A1628] to-[#0D2240]'
                  }`}>
                    <div className="absolute inset-0 flex items-center justify-center">
                      {pub.tipo === 'noticia' && <Newspaper className="h-16 w-16 text-white/20" />}
                      {pub.tipo === 'evento' && <Calendar className="h-16 w-16 text-[#0A1628]/20" />}
                    ...
                    {pub.destacado && (
                      <Badge className="absolute top-4 right-4 bg-[#D4A017] text-[#0A1628]">
                        DESTACADO
                      </Badge>
                    )}
                  </div>
                  <CardContent className="p-6">
                    ...
                    <h3 className="text-xl font-bold text-[#0A1628] mb-3 group-hover:text-[#D4A017] transition-colors line-clamp-2">
                      {pub.titulo}
                    </h3>
                    <p className="text-gray-600 mb-4 line-clamp-2">
                      {pub.resumen}
                    </p>
                    <span className="text-[#D4A017] font-medium inline-flex items-center gap-1 group-hover:gap-2 transition-all">
              ...
              >
                <Card className="overflow-hidden hover:shadow-lg transition-all h-full flex flex-col">
                  ...
                  <CardContent className="p-5 flex-1 flex flex-col">
                    ...
                    <h3 className="font-bold text-[#0A1628] mb-2 group-hover:text-[#D4A017] transition-colors line-clamp-2">
                      {pub.titulo}
                    </h3>
